...
$PatchScriptFolder = "\\52prpr-fs-001\Patching\Plugins"
#Patch Script Logs Folder - Where the patch logs, setup files, and script running logs are located.
$PatchScriptLogs = "$PatchScriptFolder\MST Patch Logs\"
...
Function ArrayInit([ref]$Array)
 ...
 Function Ping-Check
     ...
     (($object.Send("$i", $timeout)).Status -eq 'Success') 
     }
...
#Computer List File Popup
    ...
    $OpenFileDialog.initialDirectory = "$PatchScriptFolder\ComputerLists\"
...
#Patch .cmd variables
    If ($userinput -eq "yes")
    {$PatchCmd = Get-ChildItem -Path $KickerPath -filter *.ps1 -name}
    Else
    {$PatchCmd = Get-ChildItem -Path $FolderPath -filter *.ps1 -name}  
...
#Start WOL
    ...
    Write-Output "Checking List for current IPs"
        ...
        $PatchArray[$Counter] = $Patch                            
                ...
                #Read Audit logs
			    $AuditData = "\\prqens20vdm1\McConnell_Public\Patching\Audit_Logs"
				...
				If ($AD -eq $true)
					...
					$TrueIP = ($IP[$Counter] -Match "132.61.")
					...
					If ($TrueIP -eq $true) 
						...
						If ($AM -eq $true)
				            ...
				            #Run mc-wol.exe
				            $WolCmd = "\\prqens20vdm1\McConnell_Public\Patching\mc-wol.exe"
    ...
    #Display total WOL sent
		...
		Else
			{
			Write-Output "Waiting 1/60 minutes for systems to start`n"
			start-sleep -s 1
			}

	#Start Ping
		$DupInd=$SysDone=$SysPatched=$Counter=$AlreadyPatched = 0
            ...
            If ($DupArray[$Counter] -eq "0")
                {
                    If ($Online -eq "Online")
                        ...
                        $PatcherPsExec = "\\$i powershell.exe -f $PSExecCmdPath$FolderName\$PatchCmd"
                        ...
                        While (-Not $runspace.IsCompleted) {Start-Sleep -s 1; $timer += 1; $runspacecomplete = ($timer -eq 60); If ($runspacecomplete -eq $true) {Write-output "Access denied to C$, Did not patch.";$APDisplay = "Access denied to C$";$timedout = $true; break}}
